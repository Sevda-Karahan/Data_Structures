...
typedef struct TrieNode{
	...
	char digit;
	int isEndOfWord;
	struct TrieNode *child[10]; // 0-9 arasý 10 düðüm
} TrieNode;
...
int main() {
	...
	// Sözlüðe ait dosyayý oku ve Trie'ye kelime ekle
	...
	// Sorgu için sayýyý al ve kelime karþýlýklarýný yazdýr
...
void insertWords(TrieNode *root, const char *word, struct Keypad *keypad) {
	...
	//Kelimenin uzunluðunda dönerek, sýrasýyla her bir harfe karþýlýk gelen sayýyý buluyoruz.
	for (i = 0; i < length; i++) {
	    currentLetter = word[i];
	    found = 0;
	    
	    // Harfin hangi sayýya karþýlýk geldiðini bulma
	    j = 0;
	    while (j < 10 && found == 0) {
	        ...
	        while ((k < lettersLength) && (keypad[j].letters[k] != toupper(currentLetter)))
	            k++;
	        //Burada keypad'deki harfler büyük harf þeklinde yazýldýðýndan, sözlükteki küçük veya büyük harfin keypad'deki sayý karþýlýðýný bulurken sýkýntý olmamasý için:
            if (k < lettersLength) {
	    ...
	    // INSERTION
	    // Yeni düðüm oluþturma gerekiyorsa
	...
	// Kelime sonunu iþaretle ve kelimeyi düðüme ata
    if(currentNode->word == NULL) {
        ...
        // Ayný sayýya denk gelen diðer kelimeler için olduðu için '#' deðerinde bir düðüm ekle
